...
class SharedViewModel(private val repository: SuggestionItemRepository, app: Application) : AndroidViewModel(app) {

    private val _data = MutableLiveData<List<Data>>().apply { value = emptyList() }
    val suggestionListData: LiveData<List<Data>> = _data

    private val _videoData = MutableLiveData<List<DataVideo>>().apply { value = emptyList() }
    val videoListData: LiveData<List<DataVideo>> = _videoData
    ...
    class ViewModelFactory(private val repository: SuggestionItemRepository, val app: Application) :
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            return SharedViewModel(repository, app) as T
        }
    }
}